...
struct CorpusTests {
  ...
  func recognises(_ written: Written) {
    ...
    #expect(
      found == written.found,
      "\(written.name): recognised \(found), the corpus names \(written.found)")
  }
  ...
  func turnsDown(_ written: Written) {
    ...
    #expect(
      turnedDown == written.declined,
      "\(written.name): turned down \(turnedDown), the corpus names \(written.declined)")
  }

  @Test(
    "a host asking about a quote it parsed itself is told what the corpus names", arguments: written
  )
  func answersAboutAQuote(_ written: Written) {
    ...
    #expect(
      reading.place(in: quote) == (said?.kind == "map" ? said : nil),
      "\(written.name): place(in:) answers about the quote differently from recognise")
    #expect(
      reading.unreadable(in: quote) == (said?.kind == "unreadable" ? said?.text : nil),
      "\(written.name): unreadable(in:) answers about the quote differently from recognise")
    #expect(
      reading.calloutClass(of: quote) == (said?.kind == "callout" ? said?.class : nil),
  ...
  @Test func theCorpusIsNotEmpty() {
    ...
    #expect(
      Self.written.allSatisfy { !$0.markdown.isEmpty }, "a case with no markdown proves nothing")
  }
}
